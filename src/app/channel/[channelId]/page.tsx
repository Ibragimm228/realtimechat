...
const Page = () => {
  ...
  const {
    ...
    sendTyping,
    handleTyping,
  } = useChatMessages({ type: "channel", id: channelId })
  const {
    input,
    setInput,
    inputRef,
    ...
    isPending,
    sendFile,
  } = useMessageInput({ type: "channel", id: channelId, encryptionKey })
      ...
      <main className="flex flex-col h-screen max-h-screen overflow-hidden bg-background text-foreground transition-all duration-500">
        <header className="border-b border-border p-2.5 flex items-center justify-between bg-card/95 backdrop-blur-md sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-1.5">
            {isAdmin && (
              <button onClick={() => { setEditName(channelName); setEditDesc(channelDescription); setIsSettingsOpen(true) }} className="p-2 text-muted-foreground hover:bg-muted rounded-full transition-colors" title="Channel Settings">
            ...
            <div key={msg.id} className="flex items-end gap-2.5 group animate-in fade-in slide-in-from-bottom-2 duration-300" style={{ animationDelay: `${Math.min(index * 20, 300)}ms` }}>
              ...
              <div className="flex flex-col max-w-[85%] min-w-0">
                <div className="relative p-2.5 rounded-2xl text-[14.5px] leading-snug break-all shadow-sm transition-all bg-card text-foreground rounded-bl-sm border border-border">
                  ...
                  <EncryptedMessage text={msg.text} encryptionKey={encryptionKey} onBurn={() => deleteMessage(msg.id)} onDecrypted={(plaintext) => onDecrypted(msg.id, plaintext)} />
                  ...
                  <div className="absolute top-1/2 -right-12 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity p-1">
                    <button onClick={() => copyMessage(msg.id)} className={`p-1.5 rounded-full bg-card hover:bg-muted shadow-sm transition-colors border border-border ${copiedMessageId === msg.id ? "text-green-500" : "text-muted-foreground"}`} title="Copy decrypted">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                    </button>
                    <button onClick={() => { setReplyTo({ id: msg.id, sender: msg.sender, text: decryptedTexts[msg.id] || "" }); inputRef.current?.focus() }} className="p-1.5 rounded-full bg-card hover:bg-muted shadow-sm transition-colors text-muted-foreground border border-border" title="Reply">
            ...
            <div className="w-full flex flex-col">
              ...
              {replyTo && (
                <div className="flex items-center gap-2 mb-2 mx-auto max-w-3xl w-full px-1 py-2 bg-muted/50 rounded-lg border-l-2 border-primary">
                  ...
                  <button onClick={() => setReplyTo(null)} className="p-1 text-muted-foreground hover:text-foreground shrink-0 mr-2">
